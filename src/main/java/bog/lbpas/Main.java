...
public class Main {
    ...
    public static void main(String[] args){
            ...
            for(String arg : args)
            {
                switch (arg.toLowerCase())
                {
                    case "debug":
                        ...
                        print.neutral("org.lwjgl.librarypath = \"" + System.getProperty("org.lwjgl.librarypath") + "\"");
                        print.neutral("java.library.path = \"" + System.getProperty("java.library.path") + "\"");
                        print.neutral("java.home = \"" + System.getProperty("java.home") + "\"");
                        print.neutral("java.vendor = \"" + System.getProperty("java.vendor") + "\"");
                        print.neutral("os.arch = \"" + System.getProperty("os.arch") + "\"");
                        print.neutral("sun.arch.data.model = \"" + System.getProperty("sun.arch.data.model") + "\"");
                        break;
                }
        ...
        try {
            iconList = List.of
                    (
                            Utils.loadAndRenderSVG(Thread.currentThread().getContextClassLoader().getResourceAsStream("textures/icon_tree_only.svg"), 16, 16, true),
                            Utils.loadAndRenderSVG(Thread.currentThread().getContextClassLoader().getResourceAsStream("textures/icon_tree_only.svg"), 32, 32, true),
                            Utils.loadAndRenderSVG(Thread.currentThread().getContextClassLoader().getResourceAsStream("textures/icon_tree_only.svg"), 48, 48, true),
                            Utils.loadAndRenderSVG(Thread.currentThread().getContextClassLoader().getResourceAsStream("textures/icon_tree_only.svg"), 64, 64, true),
                            Utils.loadAndRenderSVG(Thread.currentThread().getContextClassLoader().getResourceAsStream("textures/icon_tree_only.svg"), 128, 128, true),
                            Utils.loadAndRenderSVG(Thread.currentThread().getContextClassLoader().getResourceAsStream("textures/icon_tree_only.svg"), 256, 256, true)
                    );
        ...
        {
            GL45.glDisable(GL45.GL_DEBUG_OUTPUT);
            GL45.glDisable(GL45.GL_DEBUG_OUTPUT_SYNCHRONOUS);
        }
    ...
    private static void loadProject(String projectPath)
    {
        ...
        try (InputStream fileInputStream = Files.newInputStream(Paths.get(projectFilePath));
            ...
            while ((entry = zipIn.getNextEntry()) != null) {
                switch (entry.getName())
                {
                    ...
                    case "sceneBuilder.bin":
                    {
                        ...
                        if(data != null)
                        {
                            RLevel lvl = new SerializedResource(data).loadResource(RLevel.class);
                            if (lvl != null)
                            {
            ...
            if(mapData != null && farcData != null)
            {
                ...
                for(int i = 0; i < LoadedData.PROJECT_DATA.entries.size(); i++)
                {
                    FileDBRow row = LoadedData.PROJECT_DATA.entries.get(i);
